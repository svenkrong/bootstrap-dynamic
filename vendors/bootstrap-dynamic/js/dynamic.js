...
if (typeof EJS === "undefined") { throw new Error("Dynamic requires EJS") }
  ...
  app.res = function (doc) {
    ...
    if (doc.info.def === 'custom') {
      ...
      if (doc.info.state) {
        ...
        if(debug.ver){console.log('--> dyn.res.default - %o - app.dmq: %o - doc.info.state: %o', doc.info.title, app.dmq, doc.info.state)}
      }
    }
    ...
    var page = doc.data.page;
    var nav = setNav(page);

    if(debug.ver){console.log('page: %o', page)}
    if(debug.ver){console.log('nav: %o', nav)}
    if(debug.ver){console.log('doc.data: %o', doc.data)}

    doc.data.path = nav.root;
    ...
    switch (doc.info.type) {
      case 'static':
        html = new EJS({url: doc.data.content + doc.ejs[doc.data.base]}).render(doc.data);
        break;
      case 'dynamic':
        // mobile/desk
        if (state) {
      ...
      if (_.isObject(obj)) {
        _.map(obj, function (val, key2){
          ...
          if (!!app.dmq[key]) {
            ...
            if(debug.ver){consol.log('--> app.res - app.dmq.%s', key)}
